...
export const useDashboardStore = defineStore('dashboard', () => {
    ...
    const balanceFormatted = computed(() =>
        formatTonAmount(fromNano(balanceNano.value)),
    )
    ...
    async function fetchBalance(address: string) {
        try {
            ...
            balanceError.value =
                e instanceof Error ? e.message : 'Ошибка загрузки баланса'
        } finally {
    ...
    async function fetchTransactions(
        ...
        try {
            transactions.value = await fetchTransactionsFromApi(
                address,
                knownAddresses,
            )
            ...
            transactionsError.value =
                e instanceof Error ? e.message : 'Ошибка загрузки транзакций'
        } finally {
